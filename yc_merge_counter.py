...
class SmartObjectCounter:
    ...
    def count_objects(self):
        ...
        # If YOLO already detected objects, use those for counting to ensure correctness
        if self.object_highlighted and self.detected_objects:
            ref_box = (x1, y1, x2, y2)
            # Find the YOLO box corresponding to the selected ROI
            ref_label = None
            best_iou = 0.0
            for d in self.detected_objects:
                bx, by, bw, bh, lbl, conf = self._norm_box(d)
                iou = self._bbox_iou(ref_box, (bx, by, bx + bw, by + bh))
                if iou > best_iou:
                    best_iou = iou
                    ref_label = lbl

            # If we have a label, count boxes of same label; else count all YOLO boxes
            yolo_rects = []
            for d in self.detected_objects:
                bx, by, bw, bh, lbl, conf = self._norm_box(d)
                if (ref_label is None or ref_label == "") or (lbl == ref_label):
                    yolo_rects.append((bx, by, bw, bh))

            vis_bgr = self.original_image.copy()
            for (x, y, w, h) in yolo_rects:
            ...
            info = []
            info.append("Method used: YOLO")
            info.append(f"Objects Found: {len(yolo_rects)}")
            info.append(f"Reference Area: {max(1, (x2 - x1) * (y2 - y1))}")
